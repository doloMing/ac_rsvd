...
std::array<double, 4> gaussian_group(
    ...
    std::array<std::uint32_t, 4> bits = philox4x32(counter, key);
...
double gaussian_value(
    ...
    std::uint64_t direction,
    std::uint64_t row) {
    PhiloxKey key = make_key(seed, stream);
    std::array<double, 4> values =
        gaussian_group(key, direction, row / 4);
    return values[row % 4];
}
...
void fill_gaussian(
    ...
    // Every column in this panel uses the same Philox key.
    const PhiloxKey key = make_key(seed, stream);

    // Columns write to separate memory, so they can run in any order.
